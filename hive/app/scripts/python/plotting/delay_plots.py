...
def usage():
    ...
    print(" Collect data recovery delays from a collection of output files located in referenced directory\n")
    print(" Typical usage: python delay_plots.py --meandir=mean32 --istate=a\n")
...
def plotvalues(epoch_means, mean, terminations, directory, state):
    ...
    # plt.show()
    plt.savefig("{}-{}-{}".format("delay_replication", directory, state), prop=FontProperties().set_size('small'))
...
def process_file(filepath, avg_delay, terminated_at_acount):
    with open(filepath) as instance:
        ...
        if terminated in terminated_at_acount:
            ...
            terminated_at_acount[terminated] = 1
        # Epoch data from [0, terminated) w.r.t. delay_replication of the current simulation instance
        data = json_obj["delay_replication"][:terminated]
...
def main(directory, state):
    ...
    plotvalues(avg_delay_epoch, avg_delay_mean, [*terminated_at_acount], directory, state)
