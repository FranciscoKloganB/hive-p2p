...
def plotvalues(epoch_means, mean, terminations, directory, state):
    plt.figure()
    plt.title("Corruption Analysis - {}i{})".format(directory, state))
    plt.xlabel("Epoch")
...
def main(directory, state):
    ...
    plotvalues(avg_corrupted_parts_epoch, avg_corrupted_parts_mean, [*terminated_at_acount], directory, state)
