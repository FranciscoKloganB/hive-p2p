...
def usage():
    ...
    print(" Typical usage: python lost_parts_plots.py --meandir=mean32 --istate=a\n")
...
def plotvalues(epoch_means, mean, terminations, directory, state):
    plt.figure()
    # plt.title("Lost Parts Analysis - {}i{}".format(directory, state))
    plt.xlabel("Epoch")
    plt.ylabel("Avg. Lost Parts")
    plt.xlim(0, 720)
    plt.ylim(0, 1)
    # Trace global mean
...
def get_epochs_means(avg_lost_parts_epoch, terminated_at_acount):
    ...
    while at <= last_breakpoint:  # from 0 up to maximum of 719, inclusive
        if at == last_breakpoint:
            avg_lost_parts_epoch[at] /= divisor
            return avg_lost_parts_epoch
...
def main(directory, state):
    ...
    avg_lost_parts: List[float] = []
    avg_lost_parts_epoch: List[float] = [0.0] * 720
    terminated_at_acount: Dict[int, int] = {}
    for filename in os.listdir(path):
        _ = process_file(os.path.join(path, filename), avg_lost_parts, terminated_at_acount)
        avg_lost_parts_epoch = [sum(n) for n in zip_longest(avg_lost_parts_epoch, _, fillvalue=0)]
    # Calculate the global mean
    avg_lost_parts_mean = np.mean(avg_lost_parts)
    ...
    avg_lost_parts_epoch = get_epochs_means(avg_lost_parts_epoch, terminated_at_acount)

    plotvalues(avg_lost_parts_epoch, avg_lost_parts_mean, [*terminated_at_acount.keys()], directory, state)
