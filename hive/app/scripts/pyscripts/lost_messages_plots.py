...
def usage():
    ...
    print(" Collect data regarding lost messages from a collection of output files located in referenced directory\n")
    print(" Typical usage: python lost_messages_plots.py --meandir=mean32 --istate=a\n")
...
def plotvalues(epoch_means, mean, terminations, directory, state):
    plt.figure()
    plt.title("Lost Message Analysis - mean({}), iState({})".format(directory, state))
    plt.xlabel("Epoch")
    plt.ylabel("Avg. Number of Lost Messages")
    plt.xlim(0, 720)
    plt.ylim(0, 50)
    # Trace global mean
...
def get_epochs_means(avg_lost_messages_epoch, terminated_at_acount):
    ...
    while at <= last_breakpoint:  # from 0 up to maximum of 719, inclusive
        if at == last_breakpoint:
            avg_lost_messages_epoch[at] /= divisor
            return avg_lost_messages_epoch
...
def main(directory, state):
    ...
    avg_lost_messages: List[float] = []
    avg_lost_messages_epoch: List[float] = [0.0] * 720
    terminated_at_acount: Dict[int, int] = {}
    for filename in os.listdir(path):
        _ = process_file(os.path.join(path, filename), avg_lost_messages, terminated_at_acount)
        avg_lost_messages_epoch = [sum(n) for n in zip_longest(avg_lost_messages_epoch, _, fillvalue=0)]
    # Calculate the global mean
    avg_lost_messages_mean = np.mean(avg_lost_messages)
    ...
    avg_lost_messages_epoch = get_epochs_means(avg_lost_messages_epoch, terminated_at_acount)

    plotvalues(avg_lost_messages_epoch, avg_lost_messages_mean, [*terminated_at_acount.keys()], directory, state)
