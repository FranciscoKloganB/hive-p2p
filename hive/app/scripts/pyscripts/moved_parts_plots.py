...
def plotvalues(epoch_means, mean, terminations, directory, state):
    plt.figure()
    plt.title("Moved Parts Analysis - mean({}), iState({})".format(directory, state))
    plt.xlabel("Epoch")
    ...
    # plt.show()
    plt.savefig("{}-{}-{}".format("moved_parts", directory, state), prop=FontProperties().set_size('small'))
...
def process_file(filepath, avg_moved_parts, terminated_at_acount):
    with open(filepath) as instance:
        ...
        if terminated in terminated_at_acount:
            ...
            terminated_at_acount[terminated] = 1
        # Epoch data from [0, terminated) w.r.t. number of moved_parts of the current simulation instance
        data = json_obj["moved_parts"][:terminated]
...
def main(directory, state):
    ...
    plotvalues(avg_moved_parts_epoch, avg_moved_parts_mean, [*terminated_at_acount.keys()], directory, state)
