...
class BaseNode:
    ...
    Workers work in one or more hives
    (:py:class:`~domain.cluster_groups.BaseHive`) to try to ensure the
    durability of files belonging to the distributed backup system. Their
    route file block replicas to other workers at every epoch
    ...
    generated by
    (:py:meth:`~domain.cluster_groups.BaseHive.new_transition_matrix`).
    If every worker in the BaseHive shares their blocks with other members by
    following their respective vector then eventually, the whole BaseHive will
    reach the desired steady state distribution of file block replicas
    (:py:attr:`~domain.cluster_groups.BaseHive.v_`).
    ...
    Attributes:
        ...
        uptime:
            ...
            BaseHive as a replacement for some other BaseNode, in
            :py:meth:`~domain.cluster_groups.BaseHive._membership_maintenance`,
            the time the worker has been online is not considered. Thus,
            if a BaseNode belongs to only one BaseHive he is guaranteed to
        ...
        hives:
            A collection of :py:class:`~domain.cluster_groups.BaseHive` this
    ...
    def __init__(self, uid: str, uptime: float) -> None:
        ...
        self.hives: Dict[str, cg.BaseHive] = {}
    ...
    def set_file_routing(self,
        ...
        if isinstance(transition_vector, pd.Series):
            ...
            raise ValueError("BaseNode.set_file_routing expects a pandas.Series ",
    ...
    def remove_file_routing(self, fid: str) -> None:
        ...
        This method is called when a BaseNode is evicted from the BaseHive and
    ...
    def receive_part(self, part: FileBlockData) -> int:
        ...
        if crypto.sha256(part.data) != part.sha256:
            ...
            return HttpCodes.OK  # accepted file part, because Sha256 was correct and BaseNode did not have this replica yet

    def discard_part(self,
                     fid: str,
                     number: int,
                     corrupt: bool = False,
                     hive: cg.BaseHive = None) -> None:
    ...
    def replicate_part(self, hive: cg.BaseHive, part: FileBlockData) -> None:
        ...
        most reliable Workers in the BaseHive down to the least
        ...
        Args:
            hive:
                Gateway BaseHive that will deliver the file block replica to
    ...
    def execute_epoch(self, hive: cg.BaseHive, fid: str) -> None:
        ...
        Args:
            hive: 
                BaseHive instance that ordered execution of the epoch.
            fid:
    ...
    def get_file_parts_count(self, fid: str) -> int:
        """Counts the number of file block replicas owned by the BaseNode for a given file identifier.
    ...
    def get_epoch_status(self) -> int:
        ...
        depending on the time it has been active in the BaseHive.
    ...
    # region Overrides
    def __hash__(self):
        """Allows a BaseNode instance or a BaseNode name to be used as dict keys."""
